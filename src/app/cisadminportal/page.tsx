...
export default function AdminPage() {
  ...
  const handleAddCandidate = async () => {
    ...
    try {
      ...
      console.log("[Upload] Starting →", fileName, `(${(selectedFile.size / 1024).toFixed(1)} KB)`);
      ...
      if (insertError) {
        console.error("[Insert] Candidate insert error:", insertError);
      ...
      fetchData();
      toast({ title: "Candidate registered", description: `${newCandidate.name.trim()} has been added successfully.` });
    } catch (err: any) {
      ...
      <div className="min-h-screen bg-secondary flex items-center justify-center p-4">
        <Card className="max-w-md w-full border-none shadow-2xl overflow-hidden rounded-3xl">
          ...
          <CardHeader className="text-center pb-2">
            ...
            <CardDescription className="font-medium">Secure OTP verification required for access</CardDescription>
          </CardHeader>
          ...
          <CardContent className="space-y-6 pt-6">
            {authStep === 'email' ? (
              <div className="space-y-4">
                ...
                {authError && (
                  <p className="text-destructive text-xs font-bold text-center bg-destructive/10 p-2 rounded">
                    {authError}
                  </p>
                )}
                <Button
          ...
          <div className="bg-muted/50 p-4 text-center border-t">
            <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">
              CIS Security Protocol · 2026/2027
            </p>
          </div>
        </Card>
      </div>
      ...
      <Sidebar className="bg-secondary text-white border-none">
        <SidebarHeader className="p-6 border-b border-white/10">
          <div className="flex items-center gap-3">
            ...
            <h2 className="font-headline font-black text-xl tracking-tight uppercase">CIS Elections</h2>
          </div>
          <p className="text-white/40 text-[10px] font-bold uppercase tracking-widest mt-1">2026 / 2027</p>
        </SidebarHeader>
          ...
          <SidebarMenu>
            ...
            <SidebarMenuItem>
              <SidebarMenuButton
                ...
                className="hover:bg-accent hover:text-secondary h-12 text-accent border border-accent/20"
              >
      ...
      <SidebarInset className="bg-[#F5F5F5]">
        ...
        <header className="h-20 border-b bg-white flex items-center justify-between px-8 sticky top-0 z-40">
          ...
          <div className="flex items-center gap-6">
            ...
            <Badge
              variant={config?.is_open ? "default" : "secondary"}
              className={config?.is_open ? "bg-emerald-500" : ""}
            >
              {config?.is_open ? "POLLS OPEN" : "POLLS CLOSED"}
            </Badge>
            <Button
              size="sm"
              variant={config?.is_open ? "destructive" : "default"}
              onClick={toggleElection}
            >
              {config?.is_open
                ? <><Lock className="w-4 h-4 mr-2" /> Close Polls</>
                : <><CheckCircle2 className="w-4 h-4 mr-2" /> Open Election</>
              }
            </Button>
          </div>
        </header>
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-8">
            ...
            <TabsContent value="onboarding" className="space-y-8 m-0">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <Card className="border-none shadow-md">
                  <CardHeader>
                    <CardTitle className="text-lg flex items-center gap-2">
                      <Plus className="w-5 h-5" /> 1. Define Positions
                    </CardTitle>
                        ...
                        positions.map(p => (
                          <div key={p.id} className="flex justify-between items-center p-3 bg-muted rounded-xl">
                            ...
                            <Button
                              variant="ghost"
                              size="sm"
                              onClick={() => deleteItem('positions', p.id)}
                            >
                ...
                <Card className="border-none shadow-md">
                  <CardHeader>
                    <CardTitle className="text-lg flex items-center gap-2">
                      <UserPlus className="w-5 h-5" /> 2. Register Candidates
                    </CardTitle>
                    <CardDescription>Upload photo and assign to a position.</CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-4">
                    <Input
                    ...
                    <Button
                      ...
                      disabled={isUploading || !newCandidate.name.trim() || !newCandidate.positionId || !selectedFile}
                    >
                      {isUploading
                        ? <><Loader2 className="w-5 h-5 animate-spin mr-2" /> Uploading…</>
                        : <><Upload className="w-4 h-4 mr-2" /> Register Candidate</>
                      }
                    </Button>

                    {/* Registered candidates list */}
                    <div className="space-y-2 max-h-44 overflow-y-auto pt-4 border-t">
                      {candidates.length === 0 ? (
                        ...
                        candidates.map(c => (
                          <div key={c.id} className="flex justify-between items-center p-2 bg-muted rounded-xl">
                            <div className="flex items-center gap-3">
                              {c.photo_url ? (
                                <img
                                  src={c.photo_url}
                            ...
                            <Button
                              variant="ghost"
                              size="sm"
                              onClick={() => deleteItem('candidates', c.id)}
                            >
                              <Trash2 className="w-4 h-4 text-destructive" />
                            </Button>
                          </div>
                ...
                <Card className="border-none shadow-md md:col-span-2">
                  <CardHeader>
                    <CardTitle className="text-lg flex items-center gap-2">
                      <Users className="w-5 h-5" /> 3. Class Management & Token Generation
                    </CardTitle>
                    <CardDescription>
                      Set population and generate unique tokens (Population + 5 spare).
                    </CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <Input
                        placeholder="Class Name (e.g., Grade 6A)"
                        value={newClass.name}
                      ...
                      <Input
                        ...
                        onChange={e => setNewClass({ ...newClass, population: parseInt(e.target.value) || 0 })}
                      />
                      <Button
                        onClick={handleAddClass}
                        disabled={!newClass.name.trim() || newClass.population <= 0}
                      >
                        Add Class
                      </Button>
                    </div>
                    ...
                    {classes.length === 0 ? (
                      ...
                      <Table>
                        ...
                        <TableBody>
                          {classes.map(cls => {
                            const tokens = allTokens.filter(t => t.class_id === cls.id);
                            const target = cls.population + 5;
                            const isGenerated = tokens.length >= target;
                            const progress = cls.population > 0
                              ? Math.round(((cls.votes_cast || 0) / cls.population) * 100)
                              : 0;
            ...
            <TabsContent value="overview" className="space-y-8 m-0">
                ...
                {[
                  ...
                  <Card key={i} className="border-none shadow-sm">
                    <CardContent className="pt-6">
                      <p className="text-xs font-black text-muted-foreground uppercase mb-1">{stat.label}</p>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <Table>
                      <TableHeader className="bg-muted/30">
                        <TableRow>
                          ...
                          <TableHead className="text-xs uppercase">Votes</TableHead>
                      ...
                      <TableBody>
                        {classes.map(cls => {
                          const pct = cls.population > 0
                            ? Math.round(((cls.votes_cast || 0) / cls.population) * 100)
                            : 0;
                          const isFull = (cls.votes_cast || 0) >= cls.population;
                          return (
                            <TableRow key={cls.id}>
                              <TableCell className="font-bold">{cls.name}</TableCell>
                              ...
                              <TableCell>
                                <Badge
                                  ...
                                  className={isFull ? "bg-emerald-500" : ""}
                                >
                                  {isFull ? "Complete" : "Open"}
                                </Badge>
                              </TableCell>
                            </TableRow>
                          );
                ...
                <Card className="border-none shadow-md">
                  <CardHeader>
                    <CardTitle className="text-lg flex items-center gap-2">
                      <Sparkles className="w-5 h-5 text-accent" /> Activity Feed
                    </CardTitle>
                  </CardHeader>
                        ...
                        >
                          <div className="flex items-center gap-3">
                            ...
                            <span className="font-bold text-sm">
                              {classes.find(c => c.id === vote.class_id)?.name || 'Unknown'}
                            </span>
                          </div>
            ...
            <TabsContent value="insights" className="space-y-8 m-0">
              <div className="flex items-center justify-between">
                <div>
                  <h2 className="text-2xl font-headline font-black text-secondary uppercase">
                ...
                >
                  {isAnalyzing
                    ? <><RefreshCcw className="w-4 h-4 mr-2 animate-spin" /> Analysing…</>
              ...
              {aiInsight ? (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <Card className="border-none shadow-md">
                    <CardHeader><CardTitle className="text-lg">Turnout Prediction</CardTitle></CardHeader>
                    <CardContent>
                      <div className="text-center py-6">
                        <span className="text-6xl font-black text-primary">
                          {aiInsight.predictedFinalTurnoutPercentage}%
                        </span>
                  ...
                  <Card className="border-none shadow-md">
                    <CardHeader><CardTitle className="text-lg">Summary Analysis</CardTitle></CardHeader>
                    <CardContent>
                      <p className="text-sm leading-relaxed text-muted-foreground">
                        {aiInsight.summaryAnalysis}
                      </p>
                    </CardContent>
                  </Card>

                  <Card className="md:col-span-2 border-none shadow-md">
                    <CardHeader><CardTitle className="text-lg">Voter Engagement Strategies</CardTitle></CardHeader>
                    <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {aiInsight.engagementStrategies.map((strategy, i) => (
                        <div
                          key={i}
                          className="flex gap-4 p-4 border rounded-xl hover:bg-accent/5 transition-colors"
                        >
                          <div className="w-8 h-8 rounded-full bg-accent/10 flex items-center justify-center text-accent shrink-0 font-bold text-sm">
                            {i + 1}
                          </div>
                ...
                <div className="text-center py-20 bg-white rounded-3xl border-2 border-dashed">
                  ...
                  <p className="text-muted-foreground font-medium">
                    Click "Generate Analysis" to run the AI engine.
                  </p>
                </div>
              )}
            </TabsContent>
